...
export function UserList ({ showColors, users, changeSorting, deleteUser }: UserListProps) {
  return (
    <table width='100%'>
      <thead>
        <tr>
          <th>Picture</th>
          <th className='pointer' onClick={() => { changeSorting(SortBy.First) }}>
            Name
          </th>
            ...
            <tr key={user.login.uuid} style={{ backgroundColor: color }}>
              ...
              <td>
                <button type='button' onClick={() => { deleteUser(user.login.uuid) }}>
                  <svg xmlns='http://www.w3.org/2000/svg' fill='none' width={24} viewBox='0 0 24 24' height={24} strokeWidth={1.5} stroke='currentColor' className='w-6 h-6'>
